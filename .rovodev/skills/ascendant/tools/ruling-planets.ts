...
export const rulingPlanetsWorkflow = Effect.fn("Ascendant.rulingPlanets")(
  function* (moment: OffsetMoment, latitude: Latitude, longitude: Longitude) {
    ...
    if (d1 === undefined) {
      return yield* Effect.fail(
        new Error("KP chart generation returned no D1"),
      );
    }
    const lagnaLongitude = d1.houses[1].lagna?.longitude;
    if (lagnaLongitude === undefined) {
      return yield* Effect.fail(
        new Error("KP chart generation returned no Lagna"),
      );
    }
    const planets = Object.values(d1.houses).flatMap(
      (house) => house.planets,
    );
    const moonEntry = planets.find((planet) => planet.name === "Moon");
    if (moonEntry === undefined) {
      return yield* Effect.fail(
        new Error("KP chart generation returned no Moon"),
      );
    }
    return {
