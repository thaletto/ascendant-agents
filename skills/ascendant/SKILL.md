...
6. Read [the judgement hierarchy](references/judgement-hierarchy.md). Then read the TOON-formatted text file `charts/D1.txt` and every artifact relevant to the question: the applicable divisional chart, `dasha.txt`, relevant `yoga.txt` and `jaimini/` results, and `sav.txt` as supporting strength.
