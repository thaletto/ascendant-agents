...
6. Read [the judgement hierarchy](references/judgement-hierarchy.md). Then read `charts/D1.toon` and every artifact relevant to the question: the applicable divisional chart, `dasha.toon`, relevant `yoga.toon` and `jaimini/` results, and `sav.toon` as supporting strength.
